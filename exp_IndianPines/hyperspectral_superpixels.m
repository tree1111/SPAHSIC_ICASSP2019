        ...
        for p = x-filt:x+filt
            if(p >= 1)&&(p <= M)
                for q = y-filt:y+filt
                    ...
                    if(q >= 1)&&(q <= N)
                        if p-1>0
                            derta = derta+norm(reshape(A(p-1,q,:),1,L) - reshape(A(p,q,:),1,L));
                        end
                         if q+1<N
                            derta = derta+norm(reshape(A(p,q+1,:),1,L) - reshape(A(p,q,:),1,L));
                         end
                         if q-1>0
                            derta = derta+norm(reshape(A(p,q-1,:),1,L) - reshape(A(p,q,:),1,L));
                         end
...
temp = A(:,:,[45,30,20]);%[30,6,99] [99,67,30][20,100,140]);
for i = 1:3
